...
contract Frozymarket {
    ...
    function createMarket(
        ...
        BettingMarket memory newMarket = BettingMarket(
            msg.sender,
            name,
            resolvesBy,
            false,
    ...
    function resolveMarket(uint256 marketIndex, bool winner) public {
        require(marketIndex < markets.length, "Invalid market index");
        require(
            markets[marketIndex].resolvesBy < block.timestamp,
            "Market can not be resolved yet"
        );
        require(!markets[marketIndex].resolved, "Market has already resolved");
        require(
            msg.sender == markets[marketIndex].owner,
    ...
    function claimWinnings(uint256 marketIndex) public {
        ...
        if (markets[marketIndex].winner) {
            ...
            bpsOfPot =
                (BPS * bets[marketIndex][msg.sender][true]) /
                markets[marketIndex].totalBetsA;
            ...
            //Calc user share, in BPS for less rounding errors
            bpsOfPot =
                (BPS * bets[marketIndex][msg.sender][false]) /
                markets[marketIndex].totalBetsB;
