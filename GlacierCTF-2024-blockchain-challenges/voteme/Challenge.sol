...
contract ChallengeContract {
    ...
    constructor(address[] memory initialStakers) payable {
        require(
            msg.value >= initialStakers.length * 10 ether,
    ...
    function checkConsensus(bytes32 proposalHash) public {
        ...
        for (
            address violator = hasDuplicates(proposalHash);
            violator != address(0);
            violator = hasDuplicates(proposalHash)
        ) {
            slash(violator);
    ...
    function unstake() public {
        require(stakerMapping[msg.sender] != 0, "Not a staker");
        require(
            activeVote[msg.sender] == 0,
            "Cannot claim stake while having an active vote"
        );
    ...
    function vote(Proposal calldata proposal) public {
        ...
        require(
            activeVote[msg.sender] <= 5,
            "Can only have five votes at the same time"
        );

        bytes32 proposalHash = keccak256(
            abi.encode(proposal.target, proposal.data, proposal.value)
        );
    ...
    function purgeVotes(bytes32 proposalHash, address violator) internal {
        ...
        for (
            uint256 index = getIndex(votesMapping[proposalHash], violator);
            index != MAX_256;
            index = getIndex(votesMapping[proposalHash], violator)
        ) {
            removeElement(votesMapping[proposalHash], index);
        }
    }
